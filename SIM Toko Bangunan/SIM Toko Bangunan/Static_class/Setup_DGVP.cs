...
namespace SIM_Toko_Bangunan.Static_class
{
    class DGV
    {
        public static void Setup_DGVP(DataGridView dgv, BindingSource bs)
        {
            try
            {
                if (!(bs.Count > 0))
                ...
                {
                    col.HeaderCell.Style.Font = new Font("Segole UI", 12.0F, FontStyle.Bold, GraphicsUnit.Pixel);
                    ...
                    if (col.GetType() == typeof(DataGridViewImageColumn))
                    {
                        col.Width = 65; 
                            foreach (DataGridViewRow dgvRow in dgv.Rows)
                            {
                                ...
                                {
                                    if (((dgvRow.Cells["Columnfoto"]).Value != null) || (dgvRow.Cells["Columnfoto"].Value != System.DBNull.Value))
                                    {
                                   dgvRow.Height = 65;
